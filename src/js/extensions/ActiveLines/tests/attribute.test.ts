...
import { ActiveLines, ATTRIBUTE_ACTIVE_LINES } from '../ActiveLines';
...
describe( 'ActiveLines', () => {
  ...
  test( 'can highlight lines specified by the attribute.', () => {
    const ryuseilight = new RyuseiLight();
    document.querySelector( 'pre' ).setAttribute( ATTRIBUTE_ACTIVE_LINES, '[5, 9]' );
  ...
  test( 'can highlight lines specified by options as a number and range.', () => {
    const ryuseilight = new RyuseiLight();
    document.querySelector( 'pre' ).setAttribute( ATTRIBUTE_ACTIVE_LINES, '[3, [5, 9]]' );
